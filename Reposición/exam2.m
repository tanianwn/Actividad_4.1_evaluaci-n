...
for k = 1:N
    ...
    % Velocidad angular
    w(k) = (exp(0.3 * tk) * (1.2 * cos(tk) - 1.82 * sin(tk))) / (1 + (2 * exp(0.3 * tk) * (cos(tk) + 0.3 * sin(tk)))^2);
end
...
axis([-3 20 -15 24 0 2]); % Ingresar limites minimos y maximos en los ejes x y z [minX maxX minY maxY minZ maxZ]
